...
public:
  ...
  // ctor for a bond
  Bond(string _productId, BondIdType _bondIdType, string _ticker, double price, float _coupon, date _maturityDate);
  Bond();
...
Bond::Bond(string _productId, BondIdType _bondIdType, string _ticker,double _price, float _coupon, date _maturityDate) : Product(_productId, BOND)
{
...
double Bond::CalculatePV01() const
{
	return 1;
}
...
{
  output <<bond.GetProductId()<<","<<bond.GetPrice()<<","<< bond.ticker << "," << bond.GetMaturityDate();
  return output;
}
