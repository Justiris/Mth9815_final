...
public:
  ...
  friend ostream& operator<< (ostream &os, const Inquiry &source) {
	  if (source.side == BUY) os << source.inquiryId << "," << source.product << "," << "BUY" << "," << source.quantity << "," << source.price << endl;
	  else os << source.inquiryId << "," << source.product << "," << "SELL" << "," << source.quantity << "," << source.price << endl;
	  return os;
  }
...
/*
template<typename T>
ostream& Inquiry<T>::operator<<(ostream& os, const Inquiry<T>& source)
{
