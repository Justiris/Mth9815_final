...
public:
  ...
  friend ostream& operator<< (ostream &os, const PriceStreamOrder &source) {
	  if (source.side == BID) os << source.price << "," << source.visibleQuantity << "," << source.hiddenQuantity << "," << "BID";
	  else os << source.price << "," << source.visibleQuantity << "," << source.hiddenQuantity << "," << "OFFER";
	  return os;
  }
private:
  double price;
...
public:
  ...
  friend ostream& operator<< (ostream &os, const PriceStream &source) {
	  os << source.product << "," << "bidOrder: " << source.bidOrder << "," << "offerOrder: " << source.offerOrder;
	  return os;
  }
